...
ZTEST(stm32_cordic, test_cordic_configure_function)
{
	...
	for(int i = 0; i < ARRAY_SIZE(fn_list); i++) {
		...
		if(ll_fn == CORDIC_FUNCTION_COSINE || ll_fn == CORDIC_FUNCTION_SINE ||
		   ll_fn == CORDIC_FUNCTION_PHASE || ll_fn == CORDIC_FUNCTION_MODULUS) {
			...
			zassert_equal(ll_nres, nres_expected,
			      ...
			      CORDIC_LL_DEV->CSR);
		} else if(ll_fn == CORDIC_FUNCTION_ARCTANGENT || ll_fn == CORDIC_FUNCTION_HARCTANGENT ||
			  ll_fn == CORDIC_FUNCTION_NATURALLOG || ll_fn == CORDIC_FUNCTION_SQUAREROOT) {
	...
	const uint32_t mapped_scale_list[] = {
		LL_CORDIC_SCALE_0,
		LL_CORDIC_SCALE_1,
		LL_CORDIC_SCALE_2,
		LL_CORDIC_SCALE_3,
	...
	for(int i = 0; i < ARRAY_SIZE(scale_list); i++) {
		...
		zassert_equal(ll_scale, mapped_scale_list[i],
			      "%s: CORDIC scale 0x%x != 0x%x (expected_scale != current_scale)",
			      cordic_dev->name, mapped_scale_list[i], ll_scale);
	}
