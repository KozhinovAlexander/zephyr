...
#define AWAIT_TRIGGER_DEFAULT_TIMEOUT \
	CONFIG_OPAMP_SHELL_AWAIT_TRIGGER_DEFAULT_TIMEOUT

#define AWAIT_TRIGGER_MAX_TIMEOUT \
	CONFIG_OPAMP_SHELL_AWAIT_TRIGGER_MAX_TIMEOUT

/* Mapped 1-1 to enum opamp_trigger */
...
static int cmd_set_trigger(const struct shell *sh, size_t argc, char **argv)
{
	...
	ret = opamp_set_trigger(dev, trigger);
	if (ret < 0) {
...
static int cmd_await_trigger(const struct shell *sh, size_t argc, char **argv)
{
	...
	ret = opamp_set_trigger_callback(dev, trigger_cb, NULL);
	if (ret < 0) {
	...
	ret = opamp_set_trigger_callback(dev, NULL, NULL);
	if (ret < 0) {
...
static int cmd_trigger_is_pending(const struct shell *sh, size_t argc, char **argv)
{
	...
	ret = opamp_trigger_is_pending(dev);
	if (ret < 0) {
